...
VkPipelineShaderStageCreateInfo VulkanExampleBase::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
	...
	shaderStage.module = Cetus::tools::loadShader(fileName.c_str(), device);
...
void VulkanExampleBase::drawUI(const VkCommandBuffer commandBuffer)
{
	if (settings.overlay && UIOverlay.visible) {
		const VkViewport viewport = Cetus::initializers::viewport((float)width, (float)height, 0.0f, 1.0f);
		const VkRect2D scissor = Cetus::initializers::rect2D(width, height, 0, 0);
...
VulkanExampleBase::VulkanExampleBase(bool enableValidation)
{
	...
	if (commandLineParser.isSet("benchmark")) {
		...
		Cetus::tools::errorModeSilent = true;
	}
...
bool VulkanExampleBase::initVulkan()
{
	...
	if (commandLineParser.isSet("gpulist")) {
		...
		for (uint32_t i = 0; i < gpuCount; i++) {
			...
			std::cout << " Type: " << Cetus::tools::physicalDeviceTypeString(deviceProperties.deviceType) << "\n";
...
void VulkanExampleBase::OnUpdateUIOverlay(Cetus::UIOverlay *overlay) {}
